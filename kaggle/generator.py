...
# --- 1. SETUP GPU MODEL (Runs Locally on Kaggle, No Limits!) ---
...
# --- 3. VIDEO MAKER FUNCTION ---
def make_video(category):
    vid_num = int(time.time())
    
    if category == "LONG":
        topics = ["A mysterious vlog in a magical forest", "A poor boy works hard and becomes rich USA story", "A thief steals a magical artifact and learns a lesson", "Survival camping trip in the mountains"]
        topic = random.choice(topics)
        scenes_count = 8 # 8 clips looped = ~2 mins
        prompt = f"Write a 150-word USA English story about: {topic}. Output STRICTLY as JSON array of 8 objects: 1. 'narration': English line. 2. 'visual': 3-word English visual prompt. RAW JSON ONLY."
    else:
        # 7 Days Inverse Reality, 7 Days Funny Snake
        if day_of_year % 14 < 7:
            topic = "Inverse reality where snakes are humans and humans are pets"
        else:
            topic = "Hilarious snake encounters in modern USA houses"
        scenes_count = 4 # 4 clips looped = ~35s
        prompt = f"Write a 70-word USA English funny YouTube Shorts script about: {topic}. Output STRICTLY as JSON array of 4 objects: 1. 'narration': English line. 2. 'visual': 3-word visual prompt. RAW JSON ONLY."

    print(f"📝 Scripting {category}: {topic}")
    script_txt = ask_gemini(prompt)
    if not script_txt: return None
    ...
    if clips:
        final_video = f"{category}_USA_{vid_num}.mp4"
        ...
        return {"file": final_video, "title": title, "desc": desc, "tags": tags, "date": today_date, "id": str(vid_num), "cat": category}
    return None

# --- 4. RUN SYSTEM & PUSH TO GITHUB ---
print("🔥 KAGGLE GPU ENGINE STARTED 🔥")
v1 = make_video("LONG")
v2 = make_video("SHORT")

results = [v for v in [v1, v2] if v]
...
if results:
    ...
    for r in results: history.insert(0, r)
    with open(history_file, "w") as f: f.write(json.dumps(history))
    
    for r in results: os.system(f"cp {r['file']} myrepo/")
    ...
    os.system('git add .')
    os.system('git commit -m "Auto Update: New Daily Videos 🚀"')
    os.system('git push')
    print("✅ All Done! Videos pushed to GitHub Pages.")
